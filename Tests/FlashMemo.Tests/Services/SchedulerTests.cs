...
public class SchedulerTests
{
    ...
    private static FakeScheduleInfoCard RandomReviewCard(int? daysOverdue = null, Random? r = null)
    {
        return new()
        {
            Interval = TimeSpan.FromDays(
                (r ?? Random.Shared).NextDouble(low: 1, high: 365)),
    ...
    private static IEnumerable<FakeScheduleInfoCard> RandomReviewCards(int? daysOverdue = null)
    {
        var rand = new Random();

        for (int i = 0; i < 15; i++)
            yield return RandomReviewCard(daysOverdue, rand);
    }
